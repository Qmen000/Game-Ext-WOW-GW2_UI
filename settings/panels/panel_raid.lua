...
local function CreateAuraFilterSection(panel, profile, buffDb, debuffDb, showBuffs, showDebuffs, dependence, showPrivateAuras, privateAuraSize)
    ...
    panel:AddOptionSlider(L["Private Auras size"], L["Set the size of Private Auras on this grid."], {getterSetter = privateAuraSize, callback = function() GW.UpdateGridSettings(profile) end, min = 8, max = 40, decimalNumbers = 0, step = 1, dependence = {["RAID_FRAMES"] = true, [dependence] = true, [showPrivateAuras] = true}, groupHeaderName = L["Debuffs"], hidden = not GW.Retail or not showPrivateAuras or not privateAuraSize})
end
