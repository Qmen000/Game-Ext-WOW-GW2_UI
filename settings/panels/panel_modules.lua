...
local function LoadModulesPanel(sWindow)
    ...
    p:AddOption(DISPLAY_POWER_BARS, L["Replace the default mana/power bar."], {getterSetter = "POWERBAR_ENABLED", callback = function() if GwPlayerPowerBar then GwPlayerPowerBar:ToggleBar(); GW.UpdateClassPowerExtraManabar() end end})
    ...
    p:AddOption(L["Immersive Questing"], L["Enable the immersive questing view."], {getterSetter = "immersiveQuesting.enabled", callback = function() GW.ShowRlPopup = true end, incompatibleAddons = "ImmersiveQuesting"})
    ...
    p:AddOption(L["Arena Frames"], L["Replace the default UI group Arena frames."], {getterSetter = "ARENA_FRAMES", callback = function() GW.ShowRlPopup = true end})
    ...
    p:AddOption(L["Alert Frames"], nil, {getterSetter = "ALERTFRAME_ENABLED", callback = function() GW.ShowRlPopup = true end, hidden = GW.Classic or GW.TBC or GW.Wrath})
